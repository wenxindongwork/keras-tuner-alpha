...
class MaxTextModel(Model, ModelValidationMixin):
    ...
    def __init__(self, model_name: str, seq_len: int, global_batch_size: int, **kwargs):
    ...
    def _create_model(self):
        ...
        assert (
            global_state.get_global_attribute("distribution") is None
        ), "Should not manually set distribution when training MaxText models."
        set_global_sharding_strategy(MaxTextSharding(maxtext_config))

        model = convert_maxtext_model_to_keras_model(
            maxtext_model, self.seq_len, self.global_batch_size
        )
        self.validate_model(model)
        return model
