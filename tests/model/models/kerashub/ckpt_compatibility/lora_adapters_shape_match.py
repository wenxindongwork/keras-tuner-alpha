...
HF_HOME=/dev/shm/temp/hf KERAS_HOME=/dev/shm/temp/keras python tests/model/models/kerashub/ckpt_compatibility/lora_adapters_shape_match.py
"""
import os
...
with safe_open(
    os.path.join(golden_dir, "adapter_model.safetensors"), framework="pt", device="cpu"
) as f:
    golden_tensors = {key: f.get_tensor(key) for key in f.keys()}

# Load and compare test tensors
with safe_open(
    os.path.join(test_dir, "adapter_model.safetensors"), framework="pt", device="cpu"
) as f:
    for key in f.keys():
